...
const AdminDashboard = () => {
  ...
  const kpis = [
    ...
    },
    {
      ...
      borderColor: "border-orange-100 dark:border-orange-100/20",
    },
    {
      ...
      borderColor: "border-purple-100 dark:border-purple-100/20",
    },
  ];
      ...
      <main className="pt-32 pb-20 px-6 max-w-[1600px] mx-auto">
          ...
          {kpis.map((kpi, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 ${kpi.color}`}
                >
                  <kpi.icon size={24} />
                </div>
                <div
                  className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full ${kpi.positive ? "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10" : "text-red-600 bg-red-50 dark:bg-red-500/10"}`}
                >
                  {kpi.positive ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          <motion.div
            ...
            className="bg-white dark:bg-[#1a1c1e] p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm"
          >
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dashboardData?.charts?.finance}>
                  <defs>
                    <linearGradient
                      id="colorRevenue"
                      x1="0"
                      ...
                      y2="1"
                    >
                      <stop offset="5%" stopColor="#00B884" stopOpacity={0.1} />
                      <stop offset="95%" stopColor="#00B884" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient
                      id="colorExpense"
                      x1="0"
          ...
          >
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dashboardData?.charts?.treatments}>
                  <CartesianGrid
                    strokeDasharray="3 3"
                    vertical={false}
                    stroke="#E2E8F033"
                  ...
                  <Bar dataKey="count" radius={[10, 10, 0, 0]}>
                    {dashboardData?.charts?.treatments?.map(
                      (_: any, index: number) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="text-sm font-bold text-gray-900 dark:text-white lowercase first-letter:uppercase">
                            ...
                            {activity.target_table} #{activity.target_id}
                          </div>
                          <div className="text-xs text-gray-400 mt-0.5">
                            {new Date(activity.log_timestamp).toLocaleString()}
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-black text-gray-900 dark:text-white px-2">
              Quick Actions
            </h3>
            <div className="grid grid-cols-1 gap-4">
              <button
                onClick={() => navigate("/admin/services")}
                className="p-6 rounded-[2rem] bg-gradient-to-br from-primary-600 to-indigo-700 text-white shadow-lg shadow-primary-600/20 hover:shadow-primary-600/40 hover:-translate-y-1 transition-all text-left"
              >
                <Activity size={32} className="mb-4 opacity-80" />
                <div className="text-lg font-black leading-tight">
                  Manage Service Tracks
                </div>
                <div className="text-sm font-medium opacity-70 mt-1">
                  Configure departments, plans & logic
                </div>
              </button>
              <button className="p-6 rounded-[2rem] bg-white dark:bg-[#1a1c1e] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all text-left group">
                <Users
                  size={32}
                  className="mb-4 text-primary-600 group-hover:scale-110 transition-transform"
                />
              ...
              <button className="p-6 rounded-[2rem] bg-white dark:bg-[#1a1c1e] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all text-left group">
                <BarChart3
                  size={32}
                  className="mb-4 text-purple-600 group-hover:scale-110 transition-transform"
                />
                <div className="text-lg font-black text-gray-900 dark:text-white leading-tight">
                  Financial Reports
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setShowLogoutConfirm(false)}
                  className="flex-1 py-2.5 bg-[#e0e2ec] dark:bg-[#43474e] text-[#1a1c1e] dark:text-[#e3e2e6] rounded-full text-xs font-bold"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#ba1a1a] text-white rounded-full text-xs font-bold shadow-md"
                >
                  Logout
                </button>
              </div>
