...
import { 
    Plus, Search, Trash2, Zap, Activity, Moon, Sun, LogOut, 
    Settings, CreditCard, Clock, ChevronDown, Info, 
    Box, User, Check, Eye, Stethoscope, UserPlus, 
    Syringe, Microscope, Dna, FlaskConical, Brain, HandHelping, ShieldPlus, Bone, Waves, Timer, HeartPulse, Pill, Calendar
} from 'lucide-react';
...
const IconComponent = ({ name, size = 20, strokeWidth = 2, className = "" }: { name: string, size?: number, strokeWidth?: number, className?: string }) => {
...
const ServiceManager = () => {
    ...
    const [activeTab, setActiveTab] = useState<'identity' | 'treatment' | 'schedule' | 'permissions'>('identity');
    const [previewSelectedPlanId, setPreviewSelectedPlanId] = useState<string>('');
    const [isLoading, setIsLoading] = useState(true);

    const [tracks, setTracks] = useState<ServiceTrack[]>([]);
    const [paymentMethods, setPaymentMethods] = useState<{method_code: string, method_name: string}[]>([]);
    ...
    const handleAddNew = () => {
        ...
        setFormData({
            ...
            scheduling: { enabled: false, slotInterval: 60, slotCapacity: 1, startTime: '09:00', endTime: '18:00' },
            permissions: {
                allowDiscount: false,
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to permanently delete this service track?')) return;
        
        try {
            const response = await authFetch(`${API_BASE_URL}/admin/services?id=${id}`, {
                method: 'DELETE'
            });
            const data = await response.json();
            if (data.status === 'success') {
    ...
    const shortcuts: ShortcutItem[] = [
        ...
        { keys: ['Alt', 'N'], description: 'Add New Track', group: 'Actions', action: handleAddNew },
    ...
    useEffect(() => {
        if (isEditorOpen && formData.pricing?.plans?.length && !previewSelectedPlanId) {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.altKey) {
                ...
                if (e.key.toLowerCase() === 's') { e.preventDefault(); setShowGlobalModal(true); }
                if (e.key.toLowerCase() === 'n') { e.preventDefault(); handleAddNew(); }
                if (e.key.toLowerCase() === 'w') { e.preventDefault(); toggleTheme(); }
                if (e.key.toLowerCase() === 'l') { e.preventDefault(); setShowLogoutConfirm(true); }
            }
        };
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FF] dark:bg-[#0F1117] text-[#1C1B1F] dark:text-[#E6E1E5] transition-colors duration-300">
                ...
                <div className="h-full px-6 flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        ...
                        <nav className="hidden lg:flex items-center gap-1 bg-gray-100/50 dark:bg-gray-800/50 p-1 rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
                            {[
                                ...
                                { label: 'Service Tracks', path: '/admin/services', active: true },
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative">
                            <button onClick={logout} className="w-10 h-10 flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-full transition-all">
                                <LogOut size={20} />
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16 px-4">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[11px] font-black uppercase tracking-[0.4em] text-[#6366f1]/80">System Infrastructure / Blueprints</span>
                        </div>
                        <h2 className="text-5xl font-black text-gray-900 dark:text-white tracking-tighter leading-none mb-3">Service Tracks</h2>
                        <p className="text-lg text-gray-400 dark:text-gray-500 font-medium max-w-2xl">Engineer custom clinical form experiences and synchronized operational logic for department-specific patient tracks.</p>
                    </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                    {isLoading ? (
                        <div className="col-span-full py-32 flex flex-col items-center justify-center space-y-4">
                            ...
                            <p className="text-xs font-black uppercase tracking-widest text-gray-400">Syncing with Blueprint Registry...</p>
                        </div>
                    ) : (
                            ...
                            {tracks.map((track: ServiceTrack, idx: number) => (
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex items-start justify-between mb-10">
                                        <div className="w-20 h-20 rounded-3xl flex items-center justify-center shadow-2xl shadow-black/5 ring-1 ring-black/5 dark:ring-white/10 transition-all group-hover:scale-110 group-hover:-rotate-3" style={{ backgroundColor: `${track.themeColor}15`, color: track.themeColor }}>
                                    ...
                                    <div className="relative z-10 mb-10">
                                        <h3 className="text-3xl font-black text-gray-900 dark:text-white group-hover:text-[#6366f1] transition-colors uppercase leading-[0.9] tracking-tighter mb-4">{track.name}</h3>
                                        <div className="flex items-center gap-2">
                                            <div className="px-2 py-0.5 rounded-md bg-[#6366f1]/10 text-[#6366f1] text-[8px] font-black uppercase tracking-widest">Interface</div>
                                            <p className="text-[10px] font-black tracking-widest text-gray-400 uppercase">{track.buttonLabel || 'DEFAULT'}</p>
                                        </div>
                                    </div>
                                    ...
                                    <button onClick={() => handleEdit(track)} className="relative z-10 w-full h-12 rounded-2xl bg-gray-50 dark:bg-gray-800 text-[10px] font-black uppercase tracking-widest text-gray-500 group-hover:bg-[#6366f1] group-hover:text-white transition-all">
                                        Refine Logic
                                    </button>
                                </motion.div>
                            ))}
                    ...
                    {!isLoading && tracks.length === 0 && (
                        <div className="col-span-full py-32 flex flex-col items-center justify-center opacity-30 select-none text-center">
                            ...
                            <p className="text-sm font-black uppercase tracking-[0.4em] text-gray-400">Blueprint Registry Empty</p>
                        </div>
                    )}
                </div>
            </main>
            ...
            <AnimatePresence>
                {isEditorOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsEditorOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-md" />
                        <motion.div 
                        ...
                        >
                            ...
                            <div className="px-10 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                                <div>
                                    <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">
                                        {editingTrack ? `Edit Track: ${formData.name}` : 'Blueprint New Service Track'}
                                    </h2>
                                    <div className="flex gap-4 mt-2">
                                        {[
                                            { id: 'identity', label: 'Identity', icon: User },
                                            { id: 'treatment', label: 'Treatment Plans', icon: Activity },
                                            { id: 'schedule', label: 'Schedule & Logic', icon: Clock },
                                ...
                                <button onClick={() => setIsEditorOpen(false)} className="w-12 h-12 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-all text-gray-400">
                                    <X size={28} />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 overflow-hidden flex bg-gray-50/50 dark:bg-black/20">
                                                    ...
                                                    <div className="bg-white dark:bg-[#1C1B22] p-8 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
                                                        <div className="grid grid-cols-2 gap-6">
                                                            <div className="relative group">
                                                                <span className="absolute -top-2 left-4 px-2 bg-white dark:bg-[#1C1B22] text-[10px] font-black uppercase tracking-widest text-[#6366f1]">Track Name</span>
                                                                <input 
                                                                    type="text"
                                                                    value={formData.name || ''}
                                                            ...
                                                            <div className="relative group">
                                                                <span className="absolute -top-2 left-4 px-2 bg-white dark:bg-[#1C1B22] text-[10px] font-black uppercase tracking-widest text-[#6366f1]">Registration Button Label</span>
                                                                <input 
                                                                    type="text"
                                                        ...
                                                        <div className="space-y-4">
                                                            <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Brand Identity</span>
                                                            <div className="grid grid-cols-12 gap-6 items-center">
                                                                <div className="col-span-4 flex items-center gap-4 h-16 px-4 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20">
                                                                    <div className="w-10 h-10 rounded-lg shadow-sm border border-white/20" style={{ backgroundColor: formData.themeColor }}></div>
                                                                    <div className="flex-1 flex flex-col">
                                                                        <span className="text-[9px] font-black text-gray-400 uppercase">Theme Color</span>
                                                                        <input type="color" value={formData.themeColor} onChange={(e) => setFormData({...formData, themeColor: e.target.value})} className="w-full h-6 rounded cursor-pointer bg-transparent border-none p-0" />
                                                                    </div>
                                                                </div>
                                                ...
                                                <motion.div key="treatment" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                                                    <div className="bg-white dark:bg-[#1C1B22] p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                                                        <div className="flex items-center justify-between mb-8">
                                                            <div className="flex items-center gap-4">
                                                                <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center text-emerald-600"><CreditCard size={24} /></div>
                                                                <div>
                                                                    <h4 className="text-lg font-black text-gray-900 dark:text-white">Billing Strategy</h4>
                                                                    <p className="text-xs text-gray-500 font-medium">How should charges be calculated?</p>
                                                                </div>
                                                            </div>
                                                            <label className="relative inline-flex items-center cursor-pointer">
                                                                <input type="checkbox" checked={formData.pricing?.enabled} onChange={(e) => setFormData({...formData, pricing: {...formData.pricing!, enabled: e.target.checked}})} className="sr-only peer" />
                                                        ...
                                                        {formData.pricing?.enabled && (
                                                            <div className="space-y-6 border-t border-gray-50 dark:border-gray-800 pt-8">
                                                                <div className="flex items-center justify-between">
                                                                    <h5 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Active Treatment Plans</h5>
                                                                    <button 
                                                                        onClick={() => {
                                                                            const newPlan: ServicePlan = { id: Math.random().toString(), icon: 'Zap', name: 'New Plan', subtitle: 'Brief description', rate: 0, days: 1 };
                                                                            setFormData({...formData, pricing: {...formData.pricing!, plans: [...(formData.pricing!.plans || []), newPlan]}});
                                                                        }}
                                                                ...
                                                                <div className="space-y-4">
                                                                    {(formData.pricing.plans || []).map((plan, idx) => (
                                                                        <div key={plan.id} className="bg-gray-50 dark:bg-black/20 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-4 group">
                                                                            <div className="flex gap-4">
                                                                                <div className="flex-1 space-y-4">
                                                                                    <div className="grid grid-cols-2 gap-4">
                                                                                        <input 
                                                                                            ...
                                                                                            onChange={(e) => {
                                                                                                const newPlans = [...formData.pricing!.plans];
                                                                                                newPlans[idx] = { ...plan, name: e.target.value };
                                                                                                setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                            }}
                                                                                        ...
                                                                                        <input 
                                                                                            ...
                                                                                            onChange={(e) => {
                                                                                                const newPlans = [...formData.pricing!.plans];
                                                                                                newPlans[idx] = { ...plan, subtitle: e.target.value };
                                                                                                setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                            }}
                                                                                    ...
                                                                                    <div className="grid grid-cols-2 gap-4">
                                                                                        <div className="space-y-1">
                                                                                            ...
                                                                                            <input 
                                                                                                ...
                                                                                                onChange={(e) => {
                                                                                                    const newPlans = [...formData.pricing!.plans];
                                                                                                    newPlans[idx] = { ...plan, rate: Number(e.target.value) };
                                                                                                    setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                                }}
                                                                                        ...
                                                                                        <div className="space-y-1">
                                                                                            <span className="text-[10px] font-black uppercase text-gray-400">Total Days</span>
                                                                                            <input 
                                                                                                type="number" 
                                                                                                value={plan.days} 
                                                                                                onChange={(e) => {
                                                                                                    const newPlans = [...formData.pricing!.plans];
                                                                                                    newPlans[idx] = { ...plan, days: Number(e.target.value) };
                                                                                                    setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                                }}
                                                                                                className="w-full bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-gray-100 dark:border-gray-800 text-xs font-black outline-none focus:ring-2 focus:ring-[#6366f1]/20"
                                                                                            />
                                                                                    ...
                                                                                    <div className="space-y-3">
                                                                                        <span className="text-[10px] font-black uppercase text-gray-400">Card Icon selection (medical focused)</span>
                                                                                        <div className="flex flex-wrap gap-2 p-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800">
                                                                                            {AVAILABLE_ICONS.filter(i => ['Stethoscope', 'HeartPulse', 'Syringe', 'Microscope', 'Dna', 'FlaskConical', 'Brain', 'HandHelping', 'ShieldPlus', 'Bone', 'Waves', 'Timer', 'Pill', 'Activity', 'Zap', 'Clock', 'Box', 'Plus'].includes(i.name)).map(i => (
                                                                                                <button 
                                                                                                    key={i.name}
                                                                                                    onClick={() => {
                                                                                                        const newPlans = [...formData.pricing!.plans];
                                                                                                        newPlans[idx] = { ...plan, icon: i.name };
                                                                                                        setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                                    }}
                                                                                ...
                                                                                <button 
                                                                                    onClick={() => {
                                                                                        const newPlans = formData.pricing!.plans.filter((_, i) => i !== idx);
                                                                                        setFormData({...formData, pricing: {...formData.pricing!, plans: newPlans}});
                                                                                    }}
                                            ...
                                            {activeTab === 'schedule' && (
                                                <motion.div key="schedule" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                                                   <div className="bg-white dark:bg-[#1C1B22] p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                                                        <div className="flex items-center justify-between mb-8">
                                                            <div className="flex items-center gap-4">
                                                                <div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600"><Clock size={24} /></div>
                                                                <div>
                                                                    <h4 className="text-lg font-black text-gray-900 dark:text-white">Scheduling Engine</h4>
                                                                    <p className="text-xs text-gray-500 font-medium">Slot management blueprint.</p>
                                                                </div>
                                                            </div>
                                                            <label className="relative inline-flex items-center cursor-pointer">
                                                                <input type="checkbox" checked={formData.scheduling?.enabled} onChange={(e) => setFormData({...formData, scheduling: {...formData.scheduling!, enabled: e.target.checked}})} className="sr-only peer" />
                                                            ...
                                                            <div className="grid grid-cols-2 gap-8 pt-8 border-t border-gray-50 dark:border-gray-800">
                                                                <div className="space-y-4">
                                                                    <div className="flex justify-between items-center px-1">
                                                                        <span className="text-[10px] font-black uppercase text-gray-400">Slot Duration</span>
                                                                        <span className="text-xs font-black text-[#6366f1]">{formData.scheduling.slotInterval} mins</span>
                                                                    </div>
                                                                    <input 
                                                                        type="range" min="15" max="120" step="15" 
                                                                        value={formData.scheduling.slotInterval}
                                                                        onChange={(e) => setFormData({...formData, scheduling: {...formData.scheduling!, slotInterval: parseInt(e.target.value)}})}
                                                                        className="w-full h-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg appearance-none cursor-pointer accent-[#6366f1]"
                                                                    />
                                                                </div>
                                                                <div className="space-y-4">
                                                                    <div className="flex justify-between items-center px-1">
                                                                        <span className="text-[10px] font-black uppercase text-gray-400">Max Capacity</span>
                                                                        <span className="text-xs font-black text-[#6366f1]">{formData.scheduling.slotCapacity} Pat/Slot</span>
                                                                    </div>
                                                                    <input 
                                                                        type="range" min="1" max="50" step="1" 
                                                                        value={formData.scheduling.slotCapacity}
                                                                        onChange={(e) => setFormData({...formData, scheduling: {...formData.scheduling!, slotCapacity: parseInt(e.target.value)}})}
                                                                        className="w-full h-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg appearance-none cursor-pointer accent-[#6366f1]"
                                                                    />
                                                                </div>
                                                                <div className="col-span-1 space-y-2">
                                                                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Daily Start Time</label>
                                                                    <input 
                                                                        type="time" value={formData.scheduling.startTime}
                                                                        onChange={(e) => setFormData({...formData, scheduling: {...formData.scheduling!, startTime: e.target.value}})}
                                                                ...
                                                                <div className="col-span-1 space-y-2">
                                                                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Daily End Time</label>
                                                                    <input 
                                                                        type="time" value={formData.scheduling.endTime}
                                                                        onChange={(e) => setFormData({...formData, scheduling: {...formData.scheduling!, endTime: e.target.value}})}
                                                                        className="w-full h-12 px-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-sm font-bold"
                                                                    />
                                                                </div>
                                                            </div>
                                            ...
                                            {activeTab === 'permissions' && (
                                                <motion.div key="permissions" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                                                    <div className="bg-white dark:bg-[#1C1B22] p-8 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm space-y-8">
                                                        <div>
                                                            <h4 className="text-sm font-black text-gray-900 dark:text-white mb-4 uppercase tracking-widest">Financial Permissions</h4>
                                                            <div className="grid grid-cols-2 gap-6">
                                                                <div className="p-5 rounded-2xl bg-gray-50/50 dark:bg-black/20 border border-gray-100 dark:border-gray-800 flex items-center justify-between">
                                                                    <div>
                                                                        <p className="text-xs font-black text-gray-900 dark:text-white uppercase">Allow Discount</p>
                                                                        <p className="text-[10px] text-gray-400 font-medium">Allow manual discounts on registration</p>
                                                                    </div>
                                                                    <button 
                                                                        onClick={() => setFormData({...formData, permissions: {...formData.permissions!, allowDiscount: !formData.permissions?.allowDiscount}})}
                                                                    ...
                                                                    >
                                                                        <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${formData.permissions?.allowDiscount ? 'left-7' : 'left-1'}`} />
                                                                    </button>
                                                                </div>
                                                                <div className="p-5 rounded-2xl bg-gray-50/50 dark:bg-black/20 border border-gray-100 dark:border-gray-800 space-y-3">
                                                                    <p className="text-xs font-black text-gray-900 dark:text-white uppercase">Max Discount %</p>
                                                                    <input 
                                                                        type="number"
                                                                        value={formData.permissions?.maxDiscountPercent || 0}
                                                                        onChange={(e) => setFormData({...formData, permissions: {...formData.permissions!, maxDiscountPercent: Math.min(100, parseInt(e.target.value) || 0)}})}
                                                        ...
                                                        <div className="p-5 rounded-2xl bg-red-50/30 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 flex items-center justify-between">
                                                            <div>
                                                                <p className="text-xs font-black text-red-600 uppercase">Approval Requirement</p>
                                                                <p className="text-[10px] text-red-400 font-medium">Require admin approval for any discount</p>
                                                            </div>
                                                            <button 
                                                                onClick={() => setFormData({...formData, permissions: {...formData.permissions!, requireDiscountApproval: !formData.permissions?.requireDiscountApproval}})}
                                                            ...
                                                            >
                                                                <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${formData.permissions?.requireDiscountApproval ? 'left-7' : 'left-1'}`} />
                                                            </button>
                                                        </div>

                                                        <div>
                                                            <h4 className="text-sm font-black text-gray-900 dark:text-white mb-4 uppercase tracking-widest">Payment Methods</h4>
                                                            <div className="grid grid-cols-2 gap-4">
                                                                 {paymentMethods.map(method => (
                                                                     <button 
                                                                         key={method.method_code}
                                                                         onClick={() => {
                                                                             const current = formData.permissions?.allowedPaymentMethods || [];
                                                                             const next = current.includes(method.method_code) ? current.filter(m => m !== method.method_code) : [...current, method.method_code];
                                                                             setFormData({...formData, permissions: {...formData.permissions!, allowedPaymentMethods: next}});
                                                                         }}
                                                                     ...
                                                                     >
                                                                         <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${formData.permissions?.allowedPaymentMethods?.includes(method.method_code) ? 'border-[#6366f1] bg-[#6366f1] text-white' : 'border-gray-200 dark:border-gray-700'}`}>
                                                                             {formData.permissions?.allowedPaymentMethods?.includes(method.method_code) && <Check size={12} />}
                                                                         </div>
                                                                         <span className="text-xs font-bold uppercase">{method.method_name}</span>
                                                                     </button>
                                                                 ))}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="p-6 rounded-3xl bg-indigo-50/30 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/50 flex items-center justify-between">
                                                            <div>
                                                                <p className="text-[11px] font-black text-indigo-600 uppercase">Allow Split Payment</p>
                                                                <p className="text-[10px] text-indigo-400 font-medium">Enable multiple payment modes for a single invoice</p>
                                                            </div>
                                                            <button 
                                                                onClick={() => setFormData({...formData, permissions: {...formData.permissions!, allowSplitPayment: !formData.permissions?.allowSplitPayment}})}
                                                            ...
                                                            >
                                                                <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${formData.permissions?.allowSplitPayment ? 'left-7' : 'left-1'}`} />
                                                            </button>
                                                        </div>
                                                    </div>
                                                </motion.div>
                                ...
                                <div className="hidden lg:block w-[850px] bg-[#F8F9FC] dark:bg-[#0F1117] overflow-y-auto custom-scrollbar p-6 border-l border-gray-100 dark:border-gray-800">
                                    <div className="mb-6 flex items-center justify-between">
                                        <div>
                                            <h3 className="text-[10px] font-black text-[#6366f1] uppercase tracking-[0.2em]">Blueprint Canvas</h3>
                                            <p className="text-[9px] text-gray-400 font-bold uppercase">Builder components only</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="rounded-[2.5rem] bg-white dark:bg-[#1A1C24] p-4 border border-gray-100 dark:border-gray-800 shadow-2xl relative overflow-hidden min-h-[800px]">
                                        ...
                                        {formData.pricing?.enabled && (
                                            <div className="space-y-4 mb-10">
                                                <div className="flex items-center gap-2 px-1">
                                                    ...
                                                    <h5 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Selected Treatment Plan</h5>
                                                </div>
                                                <div className="flex flex-wrap gap-4">
                                                    {(formData.pricing.plans || []).map((plan) => {
                                                        ...
                                                        return (
                                                            <div 
                                                                ...
                                                                style={{ borderColor: isSelected ? formData.themeColor : 'transparent' }}
                                                            >
                                                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-4 ${isSelected ? 'bg-white shadow-md' : 'bg-gray-50 dark:bg-gray-800 text-gray-400'}`} style={{ color: isSelected ? formData.themeColor : 'inherit' }}>
                                                                    <IconComponent name={plan.icon} size={18} />
                                                                </div>
                                                                <span className="text-[11px] font-black text-gray-900 dark:text-white mb-1 uppercase leading-none">{plan.name || 'Plan Name'}</span>
                                                                <span className="text-[9px] font-bold text-gray-400 leading-tight mb-4">{plan.subtitle || 'Short description'}</span>
                                                                <div className={`mt-auto w-full pt-3 border-t flex justify-between items-center ${isSelected ? 'border-indigo-100' : 'border-gray-50 dark:border-gray-800 text-gray-400'}`} style={{ color: isSelected ? formData.themeColor : 'inherit' }}>
                                                                    <span className="text-xs font-black">₹{plan.rate}</span>
                                                    ...
                                                    {(formData.pricing.plans || []).length === 0 && (
                                                        <div className="col-span-3 py-10 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-[2rem] text-center flex flex-col items-center justify-center text-gray-300">
                                                            <span className="text-[9px] font-black uppercase tracking-widest">No Plans Defined</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {(() => {
                                            const selectedPlan = formData.pricing?.plans?.find(p => p.id === previewSelectedPlanId) || formData.pricing?.plans?.[0];
                                                ...
                                                <div className="grid grid-cols-2 gap-8 mb-10">
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex items-center gap-2 px-1">
                                                            ...
                                                            <h5 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Schedule Setup</h5>
                                                        </div>
                                                        <div className="p-6 rounded-[2.5rem] bg-gray-50/50 dark:bg-black/20 border border-gray-100 dark:border-gray-800 space-y-4">
                                                            <div className="grid grid-cols-2 gap-4">
                                                                <div className="space-y-1.5">
                                                                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Rate / Day</label>
                                                                ...
                                                                <div className="space-y-1.5">
                                                                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Total Sessions</label>
                                                                    <div className="h-10 px-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center font-black text-xs">{days}</div>
                                                                </div>
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <label className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Start From Date</label>
                                                                <div className="h-11 px-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center gap-3 font-bold text-xs">
                                                                    <Calendar size={14} style={{ color: formData.themeColor }} />
                                                                    {new Date().toLocaleDateString()}
                                                                </div>
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <label className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Preferred Time Slot</label>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex items-center gap-2 px-1">
                                                            ...
                                                            <h5 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Billing Summary</h5>
                                                        </div>
                                                        <div className="p-6 rounded-[2.5rem] text-white shadow-xl space-y-6" style={{ backgroundColor: formData.themeColor, boxShadow: `0 20px 25px -5px ${formData.themeColor}33` }}>
                                                            <div className="flex justify-between items-center opacity-80">
                                                                <span className="text-[10px] font-black uppercase">Subtotal Amount</span>
                                                                <span className="text-base font-black">₹{subtotal.toFixed(2)}</span>
                                                            </div>
                                                            <div className="h-px bg-white/20 w-full" />
                                                            <div className="grid grid-cols-2 gap-4">
                                                                <div className="space-y-1">
                                                                    <span className="text-[8px] font-black uppercase opacity-60">Discount Applied</span>
                                                                    <div className="text-sm font-black text-emerald-300">₹0.00</div>
                                                                </div>
                                                                <div className="space-y-1">
                                                                    <span className="text-[8px] font-black uppercase opacity-60">Paid Amount</span>
                                                                    <div className="text-sm font-black">₹0.00</div>
                                                                </div>
                                                            </div>
                                                            <div className="pt-2">
                                                                <span className="text-[8px] font-black uppercase opacity-60">Pending Balance</span>
                                                                <div className="text-3xl font-black tracking-tighter">₹{subtotal.toFixed(2)}</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="mt-12 flex gap-4 justify-end pb-8">
                                            ...
                                            <button 
                                                ...
                                                style={{ backgroundColor: formData.themeColor, boxShadow: `0 15px 25px -5px ${formData.themeColor}33` }}
                                            >
                                                <Zap size={16} />
                            ...
                            <div className="px-10 py-2 border-t border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-black/20 flex justify-between items-center">
                                <div className="flex items-center gap-2 text-gray-400">
                                    <Info size={16} />
                                    <span className="text-[10px] font-bold uppercase tracking-widest uppercase">Draft state</span>
                                </div>
                                <div className="flex gap-4">
                                    <button onClick={() => setIsEditorOpen(false)} className="h-12 px-8 rounded-2xl text-gray-500 font-bold hover:bg-gray-100 dark:hover:bg-gray-800 transition-all text-xs uppercase tracking-widest">Discard Changes</button>
                                    <button onClick={handleSave} className="h-12 px-12 rounded-2xl bg-[#6366f1] text-white font-black text-xs uppercase tracking-[0.1em] shadow-2xl shadow-indigo-500/40 hover:scale-[1.02] active:scale-95 transition-all">
                                        Deploy Blueprint
                                    </button>
                                </div>
                            </div>
            ...
            <GlobalSearch isOpen={showGlobalModal} onClose={() => setShowGlobalModal(false)} searchQuery={globalSearchQuery} setSearchQuery={setGlobalSearchQuery} searchResults={searchResults} />
            <KeyboardShortcuts isOpen={showShortcuts} onClose={() => setShowShortcuts(false)} onToggle={() => setShowShortcuts(!showShortcuts)} shortcuts={shortcuts} />
            ...
            <AnimatePresence>
                {showLogoutConfirm && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-sm bg-white dark:bg-[#1C1B22] rounded-[2rem] p-8 shadow-2xl border border-gray-100 dark:border-gray-800 text-center">
                            ...
                            <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2">Confirm Logout</h3>
                            <p className="text-sm text-gray-500 font-medium mb-8">Are you sure you want to end your session? Any unsaved blueprint changes will be lost.</p>
                            <div className="flex gap-4">
                                <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 h-12 rounded-2xl text-gray-500 font-bold hover:bg-gray-100 dark:hover:bg-gray-800 transition-all uppercase tracking-widest text-[10px]">Cancel</button>
                                <button onClick={() => { logout(); navigate('/login'); }} className="flex-1 h-12 rounded-2xl bg-red-600 text-white font-black hover:bg-red-700 transition-all shadow-lg shadow-red-600/20 uppercase tracking-widest text-[10px]">Logout</button>
                            </div>
                        </motion.div>
                    </div>
                )}
...
const X = ({ size }: { size?: number }) => <Plus size={size} className="rotate-45" />;
