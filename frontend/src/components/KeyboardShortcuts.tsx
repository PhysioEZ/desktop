...
import { Keyboard, X, Option, Command, ArrowLeft, ArrowRight, ArrowUp, ArrowDown, Search } from 'lucide-react';
...
const KeyboardShortcuts: React.FC<KeyboardShortcutsProps> = ({ shortcuts, isOpen, onClose, onToggle }) => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            for (const shortcut of shortcuts) {
                if (shortcut.action) {
                    ...
                    const ctrlMatch = keys.includes('ctrl') === e.ctrlKey || keys.includes('control') === e.ctrlKey;
                    const metaMatch = keys.includes('meta') === e.metaKey || keys.includes('cmd') === e.metaKey;
                    const shiftMatch = keys.includes('shift') === e.shiftKey;
                    
                    const mainKey = keys.find(k => !['alt', 'ctrl', 'control', 'meta', 'cmd', 'shift'].includes(k));
                    
                    if (altMatch && ctrlMatch && metaMatch && shiftMatch && mainKey === eventKey) {
                        e.preventDefault();
                        shortcut.action();
                        if (shortcut.group !== 'General') onClose();
                        return;
                    }
                }
            }
    ...
    const renderKey = (key: string) => {
        ...
        if (key === 'Ctrl') return isMac ? <Command size={14} strokeWidth={3} /> : <span className="text-xs font-bold">Ctrl</span>;
        if (key === 'Shift') return <span className="text-xs font-bold">Shift</span>;
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {isOpen && (
                    <div className="fixed inset-0 z-[10010] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
                        <motion.div 
                        ...
                        >
                            ...
                            <div className="px-8 py-6 border-b border-[#e0e2ec] dark:border-[#43474e] flex flex-col gap-4 shrink-0 bg-[#fdfcff] dark:bg-[#1a1c1e]">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-[#e8def8] dark:bg-[#4a4458] flex items-center justify-center shrink-0">
                                            <Keyboard size={20} className="text-[#1d192b] dark:text-[#e8def8]" />
                                        </div>
                                        <h2 className="text-2xl font-normal text-[#1a1c1e] dark:text-[#e3e2e6] font-serif">Keyboard Shortcuts</h2>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button onClick={onClose} className="w-8 h-8 rounded-full hover:bg-[#f2f0f4] dark:hover:bg-[#30333b] flex items-center justify-center transition-colors ml-2">
                                            <X size={20} className="text-[#444746] dark:text-[#c4c7c5]" />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="relative max-w-full">
                                    ...
                                    />
                                    <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#74777f]" />
                                </div>
                            </div>
                            ...
                            <div className="p-8 overflow-y-auto bg-[#fdfcff] dark:bg-[#1a1c1e]">
                                <motion.div layout className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                                    ...
                                    {/* Column 1: Navigation */}
                                    {filteredAndGrouped['Navigation'] && filteredAndGrouped['Navigation'].length > 0 && (
                                        <motion.div layout className="space-y-6">
                                            <div className="space-y-3">
                                                 <h3 className="text-xs font-bold text-[#006c4c] dark:text-[#88d99d] tracking-widest uppercase border-b border-[#e0e2ec] dark:border-[#43474e] pb-2 mb-2">Navigation</h3>
                                                 <motion.div layout className="space-y-1">
                                                     <AnimatePresence>
                                                        {filteredAndGrouped['Navigation'].map((s) => (
                                                            <ShortcutRow key={getShortcutKey(s)} shortcut={s} renderKey={renderKey} />
                                                        ))}
                                                     </AnimatePresence>
                                                 </motion.div>
                                            </div>
                                    ...
                                    {(filteredAndGrouped['General'] || filteredAndGrouped['Actions']) && (
                                        <motion.div layout className="space-y-6">
                                             {filteredAndGrouped['General'] && filteredAndGrouped['General'].length > 0 && (
                                                <div className="space-y-3">
                                                     <h3 className="text-xs font-bold text-[#006c4c] dark:text-[#88d99d] tracking-widest uppercase border-b border-[#e0e2ec] dark:border-[#43474e] pb-2 mb-2">General</h3>
                                                     <motion.div layout className="space-y-1">
                                                         <AnimatePresence>
                                            ...
                                            {filteredAndGrouped['Actions'] && filteredAndGrouped['Actions'].length > 0 && (
                                                <div className="space-y-3">
                                                     <h3 className="text-xs font-bold text-[#b3261e] dark:text-[#ffb4ab] tracking-widest uppercase border-b border-[#e0e2ec] dark:border-[#43474e] pb-2 mb-2">Actions</h3>
                                                     <motion.div layout className="space-y-1">
                                                         <AnimatePresence>
                                    ...
                                    >
                                        ...
                                        {Object.keys(filteredAndGrouped).every(g => commonGroups.includes(g)) && searchQuery === '' && (
                                             <div className="text-center text-[#74777f] py-4 border-2 border-dashed border-[#e0e2ec] dark:border-[#43474e] rounded-xl">
                                                 <p className="text-xs font-medium">No page-specific shortcuts.</p>
                                             </div>
                                        )}
                                    </motion.div>
                                </motion.div>
...
const ShortcutRow = ({ shortcut, renderKey }: { shortcut: ShortcutItem, renderKey: (k: string) => React.ReactNode }) => (
    <motion.div 
        layout
    ...
    >
        <span className="text-sm text-[#1a1c1e] dark:text-[#e3e2e6] font-medium">{shortcut.description}</span>
        <div className="flex items-center gap-1.5">
            {shortcut.keys.map((key, i) => (
                <kbd key={i} className="h-8 min-w-[32px] px-2 flex items-center justify-center bg-white dark:bg-[#1a1c1e] border border-[#74777f] dark:border-[#8e918f] rounded-lg text-xs font-bold text-[#1a1c1e] dark:text-[#e3e2e6] shadow-sm font-sans mx-0.5">
                    {renderKey(key)}
                </kbd>
            ))}
        </div>
