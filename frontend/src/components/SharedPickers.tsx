...
export const InlineDatePicker = ({ value, onChange, onClose, showActions = true }: any) => {
...
export const DatePicker = ({ value, onChange, onClose }: any) => {
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10005] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="shadow-2xl rounded-[28px] overflow-hidden">
                <InlineDatePicker value={value} onChange={onChange} onClose={onClose} showActions={true} />
            </motion.div>
        </motion.div>
    );
};
