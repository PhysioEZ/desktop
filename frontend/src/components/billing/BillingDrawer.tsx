      ...
      {isDetailsModalOpen && selectedPatient && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[990]"
          />
          ...
          <motion.div
            ...
            className="fixed inset-y-0 right-0 w-full max-w-[800px] bg-white dark:bg-[#0A0A0A] shadow-2xl z-[1000] border-l border-gray-100 dark:border-[#2A2D2A] flex flex-col"
          >
            {/* Header */}
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide bg-white dark:bg-white/10 shadow-sm ${data.status === "active" || data.patient_status === "active" ? "text-green-600 dark:text-green-400" : "text-gray-500"}`}
                      >
                      ...
                      {data.due_amount
                        ...
                        : fmt(
                            Number(data.total_amount) -
                              Number(
                                data.total_paid || data.advance_payment || 0,
                              ),
                          )}
                    </p>
                  </div>
                  ...
                  {isLoadingDetails ? (
                    <div className="flex flex-col gap-3">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="h-20 w-full bg-gray-100 dark:bg-[#1C1C1C] rounded-[16px] animate-pulse"
                        />
                      ))}
                    </div>
                  ) : !data.payments || data.payments.length === 0 ? (
                    ...
                    data.payments.map((payment: any) => (
                      <div
                        key={payment.payment_id || Math.random()}
                        className="flex items-center gap-5 p-5 rounded-[20px] border border-gray-100 dark:border-[#2A2D2A] bg-white dark:bg-[#121412] hover:shadow-md transition-all group"
                      >
