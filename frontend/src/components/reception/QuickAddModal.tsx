...
const QuickAddModal = ({
  ...
  const existingPatient = useMemo(() => {
    ...
    return registration.existing_services.find(
      (s: any) => s.service_type === (isPhysio ? "physio" : "speech_therapy"),
    );
  ...
  const fetchSlots = async () => {
    ...
    try {
      ...
      const res = await authFetch(
        `${API_BASE_URL}/reception/get_treatment_slots?date=${startDate}&service_type=${serviceType}`,
      );
  ...
  const handleSave = async () => {
    ...
    if (advancePayment > 0 && Math.abs(splitTotal - advancePayment) > 0.01) {
      setError(
        `Payment split total (₹${splitTotal}) must match advance payment (₹${advancePayment})`,
      );
      return;
    }
    if (!timeSlot) {
        ...
        >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#49454f] dark:text-[#cac4d0] font-medium">
                  Patient:{" "}
                  <span className="text-[#006a6a] dark:text-[#80d4d4]">
          ...
          {existingPatient ? (
            <div className="mx-8 mb-4 p-4 bg-[#ccebc4]/20 border border-[#ccebc4] rounded-2xl flex items-center gap-3 text-[#006e1c] font-black text-sm shadow-sm ring-1 ring-[#ccebc4]/50">
              ...
              <span>
                This patient is already registered for{" "}
                {isPhysio ? "Physiotherapy" : "Speech Therapy"}. (Patient ID:{" "}
            ...
            <div className="mx-8 mb-4 p-4 bg-[#ffefc2]/30 border border-[#dec650] rounded-2xl flex items-center gap-3 text-[#675402] font-black text-sm shadow-sm ring-1 ring-[#675402]/20">
              <AlertCircle size={18} />
              <span>
                ⚠️ No patient found for this Registration ID. You can add them.
              </span>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="space-y-10">
              ...
              <section>
                <span className="text-[11px] font-bold uppercase tracking-widest text-[#006a6a] mb-4 block px-2">
                  Treatment Plan
                </span>
                  ...
                  {[
                    {
                      id: "daily",
                      title: "Daily Session",
                      desc: "Standard pay-per-day",
                      icon: <Clock size={20} />,
                      activeColor:
                        "bg-[#d8e3f4] text-[#111d2d] border-[#006a6a]",
                    },
                    {
                      id: "advance",
                      title: "Advance Care",
                      desc: "Premium priority",
                      icon: <Zap size={20} />,
                      activeColor:
                        "bg-[#f4e0f0] text-[#2d1127] border-[#9c427c]",
                    },
                    {
                      id: "package",
                      title: "RSDT Package",
                      desc: "20 sessions complete",
                      icon: <Package size={20} />,
                      activeColor:
                        "bg-[#ffedcf] text-[#2d1f11] border-[#8a5100]",
                    },
                  ]
                    .filter((p) => isPhysio || p.id !== "advance")
                    .map((p) => (
                      <button
                        key={p.id}
                      ...
                      >
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-colors ${treatmentType === p.id ? "bg-white/40" : "bg-[#1c1b1f]/8 dark:bg-[#e6e1e5]/8"}`}
                        >
                          {p.icon}
                        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-2 px-2">
                    ...
                    <h3 className="text-sm font-bold text-[#1c1b1f] dark:text-[#e6e1e5]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative group">
                      <span className="absolute -top-2 left-3 px-1 text-[10px] font-bold uppercase tracking-wider bg-[#fef7ff] dark:bg-[#141218] text-[#49454f] dark:text-[#cac4d0] z-10 transition-colors group-focus-within:text-[#006a6a]">
                        Rate / Day
                      </span>
                      <div className="relative">
                        <IndianRupee
                          size={14}
                          className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#49454f]"
                        />
                        <input
                          type="number"
                          value={costPerDay}
                          onChange={(e) =>
                            setCostPerDay(Number(e.target.value))
                          }
                    ...
                    <div className="relative group">
                      <span className="absolute -top-2 left-3 px-1 text-[10px] font-bold uppercase tracking-wider bg-[#fef7ff] dark:bg-[#141218] text-[#49454f] dark:text-[#cac4d0] z-10">
                        Total Days
                      </span>
                      <div className="relative">
                        <Hash
                    ...
                    <div className="relative group">
                      <span className="absolute -top-2 left-3 px-1 text-[10px] font-bold uppercase tracking-wider bg-[#fef7ff] dark:bg-[#141218] text-[#49454f] dark:text-[#cac4d0] z-10 transition-colors group-hover:text-[#006a6a]">
                        Start From
                      </span>
                      <div
                      ...
                      >
                        ...
                        <Calendar
                          size={16}
                          className="text-[#49454f] group-hover:text-[#006a6a] transition-colors"
                        />
                      </div>
                      <AnimatePresence>
                        {openDatePicker && (
                    ...
                    <div className="relative group">
                      <span className="absolute -top-2 left-3 px-1 text-[10px] font-bold uppercase tracking-wider bg-[#fef7ff] dark:bg-[#141218] text-[#49454f] dark:text-[#cac4d0] z-10 opacity-60">
                        End Date
                      </span>
                  ...
                  <div className="space-y-4 pt-2">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-[#006a6a] px-2 block">
                    ...
                    <div className="p-1 rounded-2xl border border-[#cac4d0] dark:border-[#49454f] overflow-hidden bg-white/40 dark:bg-black/10">
                      <div className="max-h-[220px] overflow-y-auto custom-scrollbar p-3 space-y-1">
                        {(options.paymentMethods || []).map((m: any) => (
                          <div
                            key={m.method_code}
                            className={`flex items-center justify-between p-2.5 rounded-xl transition-all ${paymentSplits[m.method_code] !== undefined ? "bg-[#006a6a]/8 border border-[#006a6a]/20" : "hover:bg-[#1c1b1f]/5 dark:hover:bg-[#e6e1e5]/5"}`}
                          >
                            <label className="flex items-center gap-3 cursor-pointer flex-1 group">
                              <div className="relative flex items-center">
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      ...
                                      const remaining = Math.max(
                                        0,
                                        advancePayment - currentTotal,
                                      );
                                      setPaymentSplits((p) => ({
                                        ...p,
                                        [m.method_code]: remaining,
                                      }));
                                    } else {
                                ...
                                />
                                <span className="absolute left-1.5 opacity-0 peer-checked:opacity-100 flex items-center justify-center text-white text-[10px]">
                                  ✓
                                </span>
                              </div>
                              <span className="text-sm font-semibold text-[#1c1b1f] dark:text-[#e6e1e5]">
                                {m.method_name}
                              </span>
                            </label>
                            {paymentSplits[m.method_code] !== undefined && (
                              <div className="relative w-36 ml-4 animate-in fade-in slide-in-from-right-2 duration-200">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-[#006a6a]">
                                  ₹
                                </span>
                                <input
                                  type="number"
                      ...
                      <div className="px-4 py-3 bg-[#f7f2fa] dark:bg-[#1c1b1f]/40 flex justify-between items-center">
                        ...
                        <div
                          className={`px-3 py-1 rounded-full text-[11px] font-bold ${Math.abs(Object.values(paymentSplits).reduce((a, b) => a + b, 0) - advancePayment) < 0.1 ? "bg-[#e7f4e8] text-[#1e4620]" : "bg-[#fceeee] text-[#8c1d18]"}`}
                        >
                          ₹
                ...
                <section className="space-y-6">
                  ...
                  <div className="space-y-5">
                    ...
                    <CustomSelect
                      ...
                      disabled={discount === 0}
                      placeholder={
                        discount > 0 ? "Select Approver" : "Not Required"
                      }
                    />

                    <div className="p-6 rounded-[24px] bg-[#006a6a]/5 border-2 border-dashed border-[#006a6a]/30 space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-[#49454f] uppercase tracking-widest">
                          Effective Amount
                        </span>
                      ...
                      <div className="flex justify-between items-center py-2 border-t border-[#006a6a]/10">
                        <div className="flex flex-col">
                          <span className="text-base font-black text-[#8c1d18]">
                            Pending Balance
                          </span>
                          <span className="text-[10px] text-[#49454f] font-medium italic">
                            Amount to be collected later
                          </span>
                        </div>
                        <span className="text-2xl font-black text-[#8c1d18] tracking-tighter">
                          ₹{calculations.due.toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
