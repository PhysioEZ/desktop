...
const SplitPaymentInput: React.FC<SplitPaymentInputProps> = ({
    ...
    const [selectedMethods, setSelectedMethods] = useState<Record<string, number>>({});
    ...
    const isFullyAllocated = Math.abs(totalAllocated - parsedTotal) < 0.01 && parsedTotal > 0;
    ...
    const toggleMethod = (methodCode: string) => {
        setSelectedMethods((prev) => {
            ...
            if (methodCode in newMap) {
                ...
                const currentTotal = parsedTotalRef.current > 0 ? parsedTotalRef.current : totalDue;
                const alreadyAllocated = Object.values(newMap).reduce((s, v) => s + v, 0);
            ...
            if (otherKeys.length > 0) {
                ...
                otherKeys.forEach((k, i) => {
                    if (i === otherKeys.length - 1) {
                        ...
                        newMap[k] = Math.max(0, Math.round((remaining - distributed) * 100) / 100);
                    } else {
                        newMap[k] = share;
                        distributed += share;
                    }
    ...
    const handleTotalChange = (value: string) => {
        ...
        setSelectedMethods((prev) => {
            ...
            if (oldTotal === 0) {
                ...
                keys.forEach((k, i) => {
                    newMap[k] = i === keys.length - 1
                        ? Math.max(0, Math.round((newTotal - share * (keys.length - 1)) * 100) / 100)
                        : share;
                });
                return newMap;
            }
            ...
            keys.forEach((k, i) => {
                if (i === keys.length - 1) {
                    newMap[k] = Math.max(0, Math.round((newTotal - distributed) * 100) / 100);
                } else {
                    const scaled = Math.round((prev[k] / oldTotal) * newTotal * 100) / 100;
                    newMap[k] = scaled;
                    distributed += scaled;
                }
            });
    ...
    return (
        <div className={`w-full rounded-2xl border overflow-hidden shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700" : "bg-white border-slate-100"}`}>
            {/* Header */}
            <div
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <div className={`px-4 pb-4 space-y-4 border-t ${isDark ? "border-slate-700" : "border-slate-100"}`}>

                            {/* Total Amount Input */}
                            <div className="pt-4">
                                <label className={`block text-[10px] font-black uppercase tracking-[0.15em] mb-2 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                                    Total Amount to Pay
                                </label>
                                <div className="relative">
                                    <span className={`absolute left-4 top-1/2 -translate-y-1/2 text-lg font-black ${isDark ? "text-teal-400" : "text-teal-600"}`}>
                                        ₹
                                    </span>
                                    <input
                                        type="number"
                                        ...
                                        className={`w-full pl-10 pr-4 py-3.5 rounded-xl text-xl font-black border-2 transition-all outline-none ${isDark
                                ...
                                {totalDue > 0 && (
                                    <p className="text-[10px] text-slate-400 mt-1.5 px-1">
                                        Outstanding balance: <span className="font-bold text-rose-400">₹{totalDue.toFixed(2)}</span>
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {paymentMethods.length === 0 ? (
                                    <p className="text-xs text-slate-400 italic text-center py-4">No payment methods available</p>
                                ) : (
                                    paymentMethods.map((m) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    {isSelected
                                                        ? <CheckCircle2 size={22} fill="currentColor" stroke="white" strokeWidth={1} />
                                                        : <Circle size={22} />
                                                    }
                                                </button>

                                                {/* Label */}
                                                <span className={`text-sm font-bold flex-1 ${isSelected ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}>
                                                    {m.method_name}
                                                </span>
                                                ...
                                                {isSelected && (
                                                    <div className="relative w-32">
                                                        <span className={`absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold ${isDark ? "text-teal-400" : "text-teal-600"}`}>
                                                            ₹
                                                        </span>
                                                        <input
                                                            type="number"
                                                            value={selectedMethods[m.method_code]}
                                                            onChange={(e) => handleAmountChange(m.method_code, e.target.value)}
                                                            className={`w-full pl-7 pr-2 py-2 rounded-lg text-right text-sm font-black border outline-none transition-all ${isDark
                            ...
                            {parsedTotal > 0 && selectedCount > 0 && (
                                <div className={`pt-3 border-t ${isDark ? "border-slate-700" : "border-slate-100"}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">Allocated</span>
                                        <span className={`text-base font-black ${isFullyAllocated ? "text-teal-500" : isOverAllocated ? "text-rose-500" : "text-amber-500"}`}>
                                            ₹{totalAllocated.toFixed(2)}
                                            <span className="text-slate-400 font-semibold text-xs ml-1">/ ₹{parsedTotal.toFixed(2)}</span>
                                        </span>
                                    </div>
                                    ...
                                    <div className={`h-1.5 rounded-full overflow-hidden ${isDark ? "bg-slate-700" : "bg-slate-100"}`}>
                                        <div
                                            ...
                                            style={{ width: `${Math.min(100, parsedTotal > 0 ? (totalAllocated / parsedTotal) * 100 : 0)}%` }}
                                        />
                                    </div>
