...
const PaymentMethodSelect = ({ value, onChange, options }: { value: string, onChange: (val: string) => void, options: string[] }) => {
...
const UpdatePaymentModal = ({ isOpen, onClose, registration, onSuccess, paymentMethods }: UpdatePaymentModalProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await authFetch(`${API_BASE_URL}/reception/registration?action=update_details`, {
                method: 'POST',
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                    <div className="px-6 py-5 border-b border-[#e0e2ec] dark:border-[#43474e] flex justify-between items-center bg-[#f2f6fa] dark:bg-[#111315]">
                        <h3 className="text-lg font-black text-[#1a1c1e] dark:text-[#e3e2e6]">Fix Payment</h3>
                        <button onClick={onClose} className="p-2 hover:bg-[#ffdad6] text-[#43474e] hover:text-[#ba1a1a] rounded-full transition-all">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        <div>
                            <label className="text-[11px] font-black text-[#43474e] dark:text-[#c4c7c5] uppercase tracking-widest block mb-2">Amount</label>
                            <div className="relative">
                        ...
                        <div>
                            <label className="text-[11px] font-black text-[#43474e] dark:text-[#c4c7c5] uppercase tracking-widest block mb-2">Payment Method</label>
                            <PaymentMethodSelect 
                                value={method} 
                                onChange={setMethod} 
