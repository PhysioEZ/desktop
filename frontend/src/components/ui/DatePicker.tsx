        ...
        >
            ...
            >
                ...
                <div className="px-6 pt-6 pb-4 border-b border-[#79747e]/10">
                    <p className="text-[#49454f] dark:text-[#cac4d0] text-[10px] font-black uppercase tracking-[0.2em] mb-1">Select date</p>
                        ...
                        {viewMode === 'calendar' ? (
                            ...
                            >
                                <div className="flex items-center justify-between px-2 mb-4">
                                    ...
                                    >
                                        {months[currDate.getMonth()]} {currDate.getFullYear()} <span className="text-[10px] opacity-70">▼</span>
                                    </button>
                                    <div className="flex gap-1">
                                        <button type="button" onClick={() => setCurrDate(new Date(currDate.getFullYear(), currDate.getMonth() - 1, 1))} className="w-10 h-10 flex items-center justify-center hover:bg-[#1d1b20]/5 rounded-full text-[#49454f] dark:text-[#cac4d0] transition-colors"><ChevronLeft size={24} /></button>
                                        <button type="button" onClick={() => setCurrDate(new Date(currDate.getFullYear(), currDate.getMonth() + 1, 1))} className="w-10 h-10 flex items-center justify-center hover:bg-[#1d1b20]/5 rounded-full text-[#49454f] dark:text-[#cac4d0] transition-colors"><ChevronRight size={24} /></button>
                                    </div>
                                </div>

                                <div className="grid grid-cols-7 text-center mb-1">
                                    {weekDays.map(d => <span key={d} className="text-[11px] font-black text-[#49454f] dark:text-[#cac4d0] w-10 h-10 flex items-center justify-center opacity-70 tracking-tighter">{d}</span>)}
                                </div>
                            ...
                            >
                                <div className="space-y-4">
                                    <p className="text-sm font-bold text-[#49454f] dark:text-[#cac4d0]">Enter date manually</p>
                                    <input 
                                        type="date"
                                        value={editValue}
                                        onChange={(e) => setEditValue(e.target.value)}
                                    ...
                                    />
                                    <p className="text-xs text-[#49454f]/60">Format: DD/MM/YYYY</p>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="flex justify-end gap-2 p-6 pt-2 shrink-0">
                    <button type="button" onClick={onClose} className="px-6 py-2.5 text-sm font-black uppercase tracking-widest text-[#6750a4] dark:text-[#d0bcff] hover:bg-[#6750a4]/5 rounded-full transition-colors active:scale-95">Cancel</button>
                    <button type="button" onClick={confirm} className="px-6 py-2.5 text-sm font-black uppercase tracking-widest text-[#6750a4] dark:text-[#d0bcff] hover:bg-[#6750a4]/5 rounded-full transition-colors active:scale-95">OK</button>
                </div>
            </motion.div>
        </motion.div>
    );
