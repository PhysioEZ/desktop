...
const Billing = () => {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (e: MouseEvent) => {
            if (notifRef.current && !notifRef.current.contains(e.target as Node) && !(e.target as Element).closest("#notif-popup"))
                ...
                <div className={`flex-1 rounded-[24px] border overflow-hidden flex flex-col shadow-sm ${isDark ? "bg-[#121412] border-[#2A2D2A]" : "bg-white border-gray-200"}`}>
                    ...
                    <div className={`flex flex-col xl:flex-row items-center justify-between gap-4 p-5 border-b ${isDark ? "border-[#2A2D2A]" : "border-gray-100"}`}>
                        ...
                        <div className="flex items-center gap-3 w-full xl:w-auto">
                            {/* Search */}
                            <div ref={searchRef} className={`relative flex items-center gap-3 px-4 py-2.5 rounded-xl border transition-all duration-300 w-full xl:w-80 ${isDark ? "bg-[#1A1C1A] border-[#2A2D2A] focus-within:border-[#4ADE80]/50" : "bg-gray-50 border-gray-200 focus-within:border-emerald-500/50 focus-within:bg-white focus-within:shadow-sm"}`}>
