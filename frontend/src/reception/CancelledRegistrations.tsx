...
import {
    ...
    Moon, Sun, TrendingUp, AlertCircle
} from 'lucide-react';
        ...
        <div className={`flex h-screen overflow-hidden font-sans transition-colors duration-300 ${isDark ? "bg-[#050505] text-slate-200" : "bg-[#FAFAFA] text-slate-900"}`}>
            ...
            <div className={`hidden xl:flex w-[450px] flex-col justify-between p-10 border-r relative shrink-0 transition-colors duration-300 z-50 ${isDark ? "bg-[#0A0A0A] border-[#151515]" : "bg-white border-gray-200"}`}>
                ...
                <div className="space-y-12 z-10">
                    <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-rose-500 ${isDark ? "bg-[#1C1C1C]" : "bg-rose-50"}`}>
                            <Trash2 size={22} />
                        </div>
                        <span className="font-semibold tracking-[0.2em] text-[10px] uppercase text-slate-500">PhysioEZ Recovery</span>
                    </div>

                    <div className="space-y-6">
                        <h1 className="text-6xl font-serif font-normal tracking-tight leading-tight text-slate-900 dark:text-slate-100">
                            Refund <br />
                            <span className={`italic ${isDark ? "text-rose-400" : "text-rose-600"}`}>Queue</span>
                        </h1>
                        <p className="text-slate-500 text-base leading-relaxed max-w-xs">
                            Manage cancelled registrations and process patient refunds efficiently.
                        </p>
                    </div>
                </div>

                <div className="space-y-10 w-full flex-1 flex flex-col justify-center py-6">
                    <div className={`p-8 rounded-[32px] border transition-all duration-300 ${isDark ? "bg-[#121212] border-white/5" : "bg-white border-rose-100 shadow-sm"}`}>
                        <div className="flex items-center gap-3 opacity-60 mb-4 text-rose-500">
                            <AlertCircle size={20} strokeWidth={2} />
                            <span className="text-xs font-semibold uppercase tracking-wider">Total Cancelled</span>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3 opacity-40 px-2 group">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Financial Recovery</span>
                        </div>
                        <div className={`p-8 rounded-[32px] border flex flex-col gap-6 ${isDark ? "bg-[#121212] border-white/5" : "bg-white border-slate-100 shadow-sm"}`}>
                            <div className="flex justify-between items-center border-b pb-6 border-dashed border-slate-200 dark:border-white/5">
                                <div>
                                    <div className="text-3xl font-medium text-emerald-600 dark:text-emerald-400">
                                        {registrations.filter(r => r.refund_status === 'initiated').length}
                                    </div>
                                    <div className="text-xs font-medium text-slate-500 mt-1">Processed</div>
                                </div>
                            ...
                            <div className="flex justify-between items-center">
                                <div>
                                    ...
                                    <div className="text-xs font-medium text-slate-500 mt-1">Awaiting Action</div>
                                </div>
            ...
            <main className="flex-1 h-screen overflow-hidden relative flex flex-col p-8 lg:p-12 gap-8">
                ...
                <div className="flex justify-between items-end shrink-0">
                    <div>
                        <h2 className="text-3xl font-medium tracking-tight text-slate-900 dark:text-slate-100">Cancelled Listings</h2>
                        <p className="text-slate-500 text-base mt-1">Track and manage service cancellations</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate('/reception/registration')} className={`px-6 py-3 border rounded-2xl flex items-center gap-3 transition-all text-xs font-bold uppercase tracking-widest ${isDark ? "bg-[#121212] border-white/5 hover:bg-white/10" : "bg-white border-slate-200 hover:bg-slate-50 shadow-sm"}`}>
                            <History size={16} /> Active Listings
                        </button>
                        <button onClick={() => fetchRegistrations()} className={`w-12 h-12 border rounded-2xl flex items-center justify-center transition-all ${isDark ? "bg-[#121212] border-white/5 hover:bg-white/10" : "bg-white border-slate-200 hover:bg-slate-50 shadow-sm"} ${isLoading ? "animate-spin" : ""}`}>
                ...
                <div className={`flex-1 rounded-[40px] border overflow-hidden flex flex-col shadow-sm transition-colors ${isDark ? "bg-[#0A0A0A] border-white/5" : "bg-white border-slate-200/60"}`}>
                    ...
                    <div className={`flex flex-col xl:flex-row items-center justify-between gap-6 p-8 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}>
                        <div className={`flex items-center gap-3 px-5 py-3 rounded-2xl border w-full xl:w-96 transition-all ${isDark ? "bg-[#121212] border-white/5 focus-within:border-rose-500/30" : "bg-slate-50 border-slate-100 focus-within:bg-white focus-within:border-rose-500/30 shadow-inner focus-within:shadow-sm"}`}>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {isLoading ? (
                            ...
                            <div className="h-64 flex flex-col items-center justify-center opacity-30 gap-4">
                                ...
                                <p className="text-xs font-bold uppercase tracking-widest">No cancelled records found</p>
                            </div>
                        ) : (
                            <div className="divide-y dark:divide-white/5 divide-slate-100">
                                {registrations.map((reg) => (
                                    <div key={reg.registration_id} className="flex items-center px-10 py-6 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-all group">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                                                {reg.phone_number} • {new Date(reg.created_at).toLocaleDateString()}
                                            </div>
                                        </div>
                                        <div className="w-[15%] text-sm font-semibold text-slate-700 dark:text-slate-300">
                                            ₹{parseFloat(reg.consultation_amount).toLocaleString('en-IN')}
                                        </div>
                                        <div className="w-[15%] flex justify-center">
                                            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest ${reg.refund_status === 'initiated' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>
                                                {reg.refund_status === 'initiated' ? 'Refunded' : 'Pending'}
                                            </span>
                                        </div>
                                        <div className="w-[15%] flex justify-center">
                                            <StatusDropdown
                    ...
                    <div className={`px-10 py-6 border-t flex items-center justify-between shrink-0 ${isDark ? "border-white/5" : "border-slate-100"}`}>
                        <div className="text-sm font-medium text-slate-500">
                            Showing <span className="text-slate-900 dark:text-white font-bold">{registrations.length}</span> entries
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold px-4">{pagination.page} / {pagination.total_pages}</span>
                            <button
                                onClick={() => fetchRegistrations(pagination.page + 1)}
            ...
            <AnimatePresence>
                {isRefundModalOpen && selectedForRefund && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsRefundModalOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={`w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden relative z-10 border ${isDark ? "bg-[#0A0A0A] border-white/5" : "bg-white border-slate-200"}`}>
                            <div className="px-8 py-6 border-b flex justify-between items-center transition-colors">
                                <div>
                                    <h3 className="text-xl font-bold tracking-tight">Process Refund</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Reg ID: #{selectedForRefund.registration_id}</p>
                                </div>
                                <button onClick={() => setIsRefundModalOpen(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full transition-all">
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleInitiateRefund} className="p-8 space-y-8">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-3">Refund Amount</label>
                                    ...
                                    <p className="text-[10px] text-rose-500 font-bold mt-3 uppercase tracking-wide">Available for refund: ₹{selectedForRefund.consultation_amount}</p>
                                </div>

                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-3">Reason for Cancellation</label>
                                    <textarea
                                        value={refundReason}
                                ...
                                >
                                    {isProcessingRefund ? <RefreshCw size={18} className="animate-spin mx-auto" /> : "Confirm Refund"}
                                </button>
                            </form>
                        </motion.div>
                    </div>
