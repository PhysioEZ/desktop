...
const ReceptionDashboard = () => {
  ...
  const handleRefresh = async () => {
    ...
    setIsLoading(true);
    const toastId = toast.loading("Performing full system sync...");

    try {
      await fetchAll(false); // We already set isLoading to true
  ...
  const handleSmartUpdate = useCallback(async () => {
    ...
    // Within 30s window - skip server hit entirely (local cache is fresh enough)
    if (diff < 30) {
      return;
    }

    // Over 30s - check sync_history in local SQLite (zero MySQL round-trips)
    // Only check tables the dashboard actually displays.
    ...
    try {
      const res = await authFetch(
        `${API_BASE_URL}/reception/check_updates?branch_id=${user.branch_id}&last_sync=${lastSync}&${dashboardTables}`,
      );
