...
const PAYMENT_METHODS = ["Cash", "Online Transfer", "GPay/PhonePe", "Credit Card", "Cheque"];
...
const Expenses = () => {
    ...
    return (
        <div className={`flex h-screen overflow-hidden font-sans transition-colors duration-300 ${isDark ? "bg-[#050505] text-slate-200" : "bg-[#FAFAFA] text-slate-900"}`}>
            ...
            {/* === LEFT PANEL (STATS) === */}
            <div className={`hidden xl:flex w-[380px] flex-col justify-between p-7 border-r relative shrink-0 transition-colors duration-300 z-50 ${isDark ? "bg-[#0A0A0A] border-[#1A1A1A]" : "bg-white border-slate-100"}`}>

                {/* Decorative background accent */}
                <div className={`absolute top-0 right-0 w-32 h-64 opacity-20 blur-[100px] pointer-events-none rounded-full ${isDark ? "bg-emerald-500/20" : "bg-emerald-500/10"}`} />

                <div className="space-y-7 z-10">
                    <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-emerald-500 ${isDark ? "bg-[#1C1C1C]" : "bg-emerald-50"}`}>
                            <PieChart size={18} />
                        </div>
                        <span className="font-bold tracking-[0.2em] text-[8px] uppercase text-slate-400">PhysioEZ Analytics</span>
                    </div>

                    <div className="space-y-3">
                        <h1 className="text-4xl font-serif font-normal tracking-tight leading-tight text-slate-900 dark:text-slate-100">
                            Expense <br />
                            <span className={`italic font-medium ${isDark ? "text-emerald-400" : "text-emerald-600"}`}>Summary</span>
                        </h1>
                        <p className="text-slate-500 text-xs leading-relaxed max-w-[240px]">
                            Branch operational performance for {format(currentDate, 'MMMM yyyy')}.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4 w-full flex-1 flex flex-col justify-center py-4 z-10">
                    ...
                    {/* Stat 1: Total Spent */}
                    <div className={`p-6 rounded-[24px] border transition-all duration-300 relative overflow-hidden group ${isDark ? "bg-[#121212] border-white/[0.03] hover:border-emerald-500/20" : "bg-white border-slate-100 shadow-sm hover:border-emerald-500/20"}`}>
                        <div className="flex items-center gap-2.5 opacity-60 mb-3">
                            <div className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-500">
                                <Wallet size={16} strokeWidth={2} />
                            </div>
                            <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Monthly Spending</span>
                        </div>
                        <div className="flex items-center justify-between gap-2">
                            <div className={`text-3xl font-semibold tracking-tight truncate ${isDark ? "text-white" : "text-slate-900"}`}>
                                {showTotal ? fmt(stats.total_spent) : '••••••'}
                            </div>
                            <button onClick={() => setShowTotal(!showTotal)} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-white/5 transition-colors text-slate-400 shrink-0">
                    ...
                    {/* Compact Budgets */}
                    <div className={`p-6 rounded-[24px] border space-y-5 ${isDark ? "bg-[#121212] border-white/[0.03]" : "bg-white border-slate-100 shadow-sm"}`}>
                        <div className="flex items-center justify-between group">
                            <div className="space-y-0.5">
                                ...
                                <div className={`text-xl font-semibold leading-none pt-1 ${isDark ? "text-white" : "text-slate-900"}`}>{fmt(stats.daily_rem)}</div>
                            </div>
                        ...
                        <div className="flex items-center justify-between group">
                            <div className="space-y-0.5">
                                ...
                                <div className={`text-xl font-semibold leading-none pt-1 text-slate-400`}>{fmt(stats.monthly_rem)}</div>
                            </div>
                    ...
                    {/* Stat 3: Allocation */}
                    <div className={`p-6 rounded-[24px] border transition-all duration-300 relative overflow-hidden ${isDark ? "bg-emerald-500/[0.02] border-emerald-500/10" : "bg-emerald-50/30 border-emerald-100"}`}>
                        <div className="flex items-center gap-2 mb-1.5">
                            <TrendingUp size={14} className="text-emerald-500" strokeWidth={2.5} />
                            <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Branch Limit</span>
                        </div>
                ...
                <div className="pt-5 border-t border-dashed border-slate-200 dark:border-white/5 flex items-center justify-between z-10">
                    <div className="flex flex-col">
                        <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-300 mb-0.5 leading-none">Security Protocol</span>
                        <span className="text-[9px] font-bold text-slate-500 uppercase leading-none">End-to-End Encrypted</span>
                    </div>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/10">
                        ...
                        <span className="text-[8px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Active</span>
                    </div>
                </div>
            </div>
            ...
            <main className="flex-1 h-screen overflow-hidden relative flex flex-col p-8 lg:p-12 gap-8">
                ...
                <div className="flex justify-between items-end shrink-0">
                    <div>
                        <h2 className="text-3xl font-medium tracking-tight text-slate-900 dark:text-slate-100">Expense Center</h2>
                        <p className="text-slate-500 text-base mt-1">Monitor clinical and operational expenditures</p>
                    </div>
                ...
                <div className={`flex-1 rounded-[40px] border overflow-hidden flex flex-col shadow-sm transition-colors ${isDark ? "bg-[#0A0A0A] border-white/5" : "bg-white border-slate-200/60"}`}>
                    ...
                    <div className={`flex flex-col xl:flex-row items-center justify-between gap-6 p-8 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}>
                        <div className="flex items-center gap-4">
                            <div className={`flex items-center gap-4 px-6 py-3.5 rounded-2xl border ${isDark ? "bg-[#121212] border-white/5" : "bg-slate-50 border-slate-100"} transition-all`}>
                                <div className="flex items-center gap-3">
                        ...
                        {loading ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center p-20 gap-6">
                                ...
                                <div className="text-center space-y-2">
                                    <h3 className="text-xl font-medium text-slate-900 dark:text-slate-200">No expenses recorded</h3>
                                    <p className="text-sm text-slate-400 font-normal">Add your first transaction for this period</p>
                                </div>
                            </div>
                        ) : (
                            <div className="divide-y dark:divide-white/5 divide-slate-100">
                                {expenses.map((exp) => (
                                    <div key={exp.id} className="flex items-center px-10 py-6 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                        <div className="w-[10%] font-mono text-xs opacity-40 font-semibold">#{exp.voucher_no || exp.id}</div>
                                        <div className="w-[12%] text-sm font-medium">{format(new Date(exp.date), 'dd MMM yyyy')}</div>
                                        <div className="w-[18%]">
                                            <div className="text-sm font-semibold uppercase tracking-tight">{exp.paid_to}</div>
                                        </div>
                                        <div className="flex-1">
                                            <div className="text-[10px] font-semibold uppercase tracking-widest text-indigo-500 mb-1">{exp.category}</div>
                                            <div className="text-sm opacity-60 line-clamp-1">{exp.reason || 'N/A'}</div>
                                        </div>
                                        ...
                                        <div className="w-[10%] flex justify-center">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest ${getStatusStyle(exp.status)}`}>
                                                {exp.status}
                                            </span>
                                        </div>
                                        <div className="w-[8%] text-right">
                                            {exp.has_bill ? (
                                                ...
                                                <div className="p-2 opacity-10"><EyeOff size={18} /></div>
                                            )}
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className={`px-10 py-6 border-t flex items-center justify-between shrink-0 ${isDark ? "border-white/5" : "border-slate-100"}`}>
                        <div className="text-sm font-medium text-slate-500">
                            Showing <span className="text-slate-900 dark:text-white font-bold">{expenses.length}</span> entries
                        </div>
                        <div className="flex items-center gap-3">
                            <button
            ...
            <AnimatePresence>
                {isAddModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            <form onSubmit={handleAddExpense} className="flex flex-col h-full">
                                {/* Modal Header */}
                                <div className="px-12 pt-12 pb-8 flex justify-between items-start">
                                    <div>
                                        <h3 className="text-[32px] font-bold tracking-tight text-[#6366f1]">Add New Expense</h3>
                                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">Create a new expense voucher</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setIsAddModalOpen(false)}
                                ...
                                <div className="px-12 pb-12 space-y-8 overflow-y-auto max-h-[75vh] custom-scrollbar">
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Voucher No.</label>
                                            <input
                                                disabled
                                        ...
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Date <span className="text-rose-500 font-bold">*</span></label>
                                    ...
                                    {/* Line 2: Paid To */}
                                    <div className="space-y-2.5">
                                        <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Paid To <span className="text-rose-500 font-bold">*</span></label>
                                        <input
                                            name="paid_to"
                                            required
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Expense Done By <span className="text-rose-500 font-bold">*</span></label>
                                            <input
                                        ...
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Category <span className="text-rose-500 font-bold">*</span></label>
                                            <select
                                                name="category"
                                                required
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Amount (₹) <span className="text-rose-500 font-bold">*</span></label>
                                            <div className="relative group">
                                                <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 font-semibold text-xl group-focus-within:text-indigo-600 transition-colors">₹</div>
                                                <input
                                                    name="amount"
                                                    type="number"
                                                    required
                                        ...
                                        <div className="space-y-2.5">
                                            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Payment Method <span className="text-rose-500 font-bold">*</span></label>
                                            <select
                                                name="payment_method"
                                                required
                                                className="w-full px-5 py-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-2xl text-base font-semibold text-slate-900 dark:text-white outline-none ring-2 ring-transparent focus:ring-indigo-500/10 focus:border-indigo-500/30 transition-all appearance-none cursor-pointer"
                                    ...
                                    <div className="space-y-2.5">
                                        <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-1">Being (Description)</label>
                                        <textarea
                                            name="reason"
                                            rows={3}
                                    ...
                                    <div className="pt-2">
                                        <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest block mb-2">Amount in Words</label>
                                    ...
                                    <div className="pt-6 border-t border-slate-200 dark:border-white/10">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-500 dark:text-slate-400"><Upload size={22} /></div>
                                                <div>
                                                    <p className="text-sm font-bold uppercase tracking-tight text-slate-900 dark:text-white">Upload Bill</p>
                                                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400">JPG, PNG or PDF supported</p>
                                                </div>
                                            </div>
                                            <input name="bill_image" type="file" className="hidden" id="bill-upload-modal" />
                                            <label htmlFor="bill-upload-modal" className="px-6 py-2.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold uppercase tracking-widest cursor-pointer hover:bg-white dark:hover:bg-white/10 transition-all text-slate-700 dark:text-slate-300">Browse Files</label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="px-12 py-10 flex justify-end items-center bg-slate-50/30 dark:bg-white/[0.01] border-t border-slate-200 dark:border-white/10">
                                    ...
                                    >
                                        {isSubmitting ? <RefreshCw size={20} className="animate-spin" /> : <><Check size={20} strokeWidth={3} /> Save Expense Record</>}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
