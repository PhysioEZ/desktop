...
const StatusDropdown = ({ currentStatus, onUpdate, getStatusColors }: { currentStatus: string, onUpdate: (val: string) => void, getStatusColors: (s: string) => string }) => {
...
const Registration = () => {
    ...
    const [selectedForPaymentFix, setSelectedForPaymentFix] = useState<RegistrationRecord | null>(null);
    ...
    useEffect(() => {
        ...
        if (!user?.branch_id || globalSearchQuery.length < 2) { setSearchResults([]); return; }
    ...
    const shortcuts: ShortcutItem[] = [
        ...
        { keys: ['Alt', 'S'], description: 'Global Search', group: 'General', action: () => setShowGlobalModal(true) },
        ...
        { keys: ['Alt', 'R'], description: 'Refresh List', group: 'Registration', action: () => { isFirstLoad.current = true; fetchRegistrations(); }, pageSpecific: true },
        { keys: ['Alt', 'F'], description: 'Focus Search', group: 'Registration', action: () => searchInputRef.current?.focus(), pageSpecific: true },
        { keys: ['Alt', 'X'], description: 'Reset Filters', group: 'Registration', action: () => { setSearchQuery(''); setStatusFilter(''); setReferrerFilter(''); setConditionFilter(''); setCurrentPage(1); }, pageSpecific: true },
        { keys: ['Alt', 'Q'], description: 'Show Pending', group: 'Registration', action: () => { setStatusFilter('pending'); setCurrentPage(1); }, pageSpecific: true },
        { keys: ['Alt', 'E'], description: 'Show Consulted', group: 'Registration', action: () => { setStatusFilter('consulted'); setCurrentPage(1); }, pageSpecific: true },
        { keys: ['Alt', 'Z'], description: 'Clear Status', group: 'Registration', action: () => { setStatusFilter(''); setCurrentPage(1); }, pageSpecific: true },
        { keys: ['Alt', 'ArrowLeft'], description: 'Previous Page', group: 'Registration', action: () => { if (currentPage > 1) setCurrentPage(p => p - 1); }, pageSpecific: true },
        { keys: ['Alt', 'ArrowRight'], description: 'Next Page', group: 'Registration', action: () => { if (currentPage < pagination.total_pages) setCurrentPage(p => p + 1); }, pageSpecific: true },
        { keys: ['Alt', 'H'], description: 'Cancelled History', group: 'Registration', action: () => navigate('/reception/registration/cancelled'), pageSpecific: true },
    ];
    ...
    const fetchOptions = useCallback(async () => {
        if (!user?.branch_id) return;
        try {
            const res = await authFetch(`${API_BASE_URL}/reception/registration`, { method: 'POST', body: JSON.stringify({ action: 'options', branch_id: user.branch_id }) });
            const data = await res.json();
            if (data.status === 'success') {
    ...
    const fetchDetails = async (id: number) => {
        try {
            const res = await authFetch(`${API_BASE_URL}/reception/registration?action=details&id=${id}`, { method: 'POST', body: JSON.stringify({ action: 'details', id }) });
            const data = await res.json();
            if (data.status === 'success') {
        ...
        <div className="min-h-screen bg-[#fdfcff] dark:bg-[#111315] text-[#1a1c1e] dark:text-[#e3e2e6] font-sans transition-colors duration-300 pb-20">
            ...
            <header className="fixed top-0 left-0 right-0 z-50 bg-[#fdfcff]/80 dark:bg-[#111315]/80 backdrop-blur-md px-4 md:px-8 py-4 flex items-center justify-between border-b border-[#e0e2ec] dark:border-[#43474e] transition-colors duration-300">
                ...
                <div className="flex items-center gap-2 lg:gap-4">
                    ...
                    <button onClick={fetchRegistrations} disabled={isSearching} className={`p-3 hover:bg-[#e0e2ec] dark:hover:bg-[#43474e] rounded-full text-[#43474e] dark:text-[#c4c7c5] transition-colors ${isSearching ? 'animate-spin' : ''}`}><RefreshCw size={22} strokeWidth={1.5} /></button>
                        ...
                        {registrations.map((reg, idx) => {
                            ...
                            >
                                    ...
                                    <div className="flex flex-col gap-1">
                                        {reg.chief_complain ? (
                                            <div className="flex items-center gap-2">
                                                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isPendingApproval ? 'bg-[#74777f]' : 'bg-[#b3261e]'}`}></span>
                                                <span className="text-sm font-semibold text-[#1a1c1e] dark:text-[#e3e2e6] truncate" title={reg.chief_complain}>{reg.chief_complain}</span>
                                            </div>
                                        ) : (
                                            <span className="text-sm text-[#43474e] dark:text-[#c4c7c5] italic">No complaint logged</span>
                                        )}
                                ...
                                <div className="col-span-2 w-full">
                                    <p className="text-xs font-medium text-[#1a1c1e] dark:text-[#e3e2e6] mb-0.5 truncate" title={reg.reffered_by}>
                                        <span className="text-[#43474e] dark:text-[#c4c7c5]">By:</span> {reg.reffered_by || 'Direct'}
                                    </p>
                                    <p className="text-[10px] text-[#43474e] dark:text-[#c4c7c5] font-medium flex items-center gap-1">
                                        <Clock size={10} /> {formatDateSafe(reg.created_at, 'MMM d, h:mm a')}
                                    </p>
                                </div>

                                {/* 4. Financial Section */}
                                <div className="col-span-2 text-left lg:text-right w-full">
                                     <p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6]">₹{reg.consultation_amount}</p>
                                     <p className="text-[10px] font-bold uppercase text-[#43474e] dark:text-[#c4c7c5] tracking-tight">{reg.payment_method || 'Unspecified'}</p>
                                </div>
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-auto p-8 pt-0 custom-scrollbar">
                                <div className="space-y-8">
                                    {/* Quick Actions */}
                                    <div className={`p-8 rounded-[32px] border border-[#d3e3fd] dark:border-[#0842a0] bg-gradient-to-br from-[#ecf3fe] to-[#d3e3fd]/30 dark:from-[#0842a0]/10 dark:to-[#0842a0]/20 relative overflow-hidden ${(selectedRegistration.approval_status === 'pending' || selectedRegistration.approval_status === 'rejected') ? 'slashed-bg' : ''}`}>
                                        <div className="flex items-center justify-between mb-8">
                                            <div>
                                                <h4 className="text-[11px] font-black text-[#041e49] dark:text-[#d3e3fd] uppercase tracking-[0.2em] flex items-center gap-2">
                                                    <Zap size={14} className="text-[#006a6a]" /> Bureau Quick Converter
                                                </h4>
                                                <p className="text-xs text-[#041e49]/70 dark:text-[#d3e3fd]/70 font-medium mt-1">Initialize treatment track conversion</p>
                                            </div>
                                        </div>
                                        
                                        {(selectedRegistration.approval_status === 'pending' || selectedRegistration.approval_status === 'rejected') && (
                                            <div className="absolute inset-0 z-10 bg-white/60 dark:bg-black/60 backdrop-blur-[2px] flex items-center justify-center p-4 text-center">
                                                 <div className="bg-[#b3261e] text-white px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl flex items-center gap-2 border border-white/20">
                                                     <Lock size={14} />
                                                     <span>{selectedRegistration.approval_status === 'pending' ? 'Pending Approval' : 'Registration Rejected'} - Actions Locked</span>
                                                 </div>
                                            </div>
                                        )}
                                            ...
                                            {serviceTracks.map((track) => {
                                                ...
                                                return (
                                                    <button 
                                                        ...
                                                        className="flex items-center gap-4 p-5 bg-white dark:bg-[#1a1c1e] border-2 border-transparent hover:border-[#006a6a]/30 rounded-[24px] hover:shadow-xl transition-all group relative overflow-hidden shadow-sm"
                                                    >
                                                        <div className="w-12 h-12 rounded-xl flex items-center justify-center group-hover:rotate-12 transition-transform shadow-sm" style={{ backgroundColor: `${track.themeColor}15`, color: track.themeColor }}>
                                                            <Icon size={22} />
                                                        </div>
                                                        <div className="text-left">
                                                            <span className="block text-sm font-black text-[#1a1c1e] dark:text-[#e3e2e6] uppercase tracking-tight">{track.buttonLabel || track.name}</span>
                                                            <span className="block text-[9px] font-bold text-[#43474e] dark:text-[#c4c7c5] mt-0.5 uppercase opacity-60">{track.name}</span>
                                                        </div>
                                                        <div className="absolute top-0 right-0 w-8 h-8 bg-[#006a6a]/5 rounded-bl-3xl translate-x-2 -translate-y-2 group-hover:translate-x-0 group-hover:translate-y-0 transition-transform"></div>
                                                    </button>
                                                );
                                        ...
                                        {selectedRegistration.patient_exists_count > 0 ? (
                                            ...
                                            >
                                                ...
                                                <span>This patient is already in the clinical registry. (Patient ID: {selectedRegistration.existing_services?.[0]?.patient_id || 'N/A'})</span>
                                            </motion.div>
                                        ) : (
                                            <motion.div 
                                                initial={{ opacity: 0, y: 10 }}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="p-8 rounded-[32px] border border-[#e0e2ec] dark:border-[#43474e] bg-white/40 dark:bg-black/10 space-y-8 relative overflow-hidden shadow-inner">
                                            ...
                                            <div className="flex items-center justify-between relative z-10"><h4 className="text-[11px] font-black text-[#006a6a] uppercase tracking-[0.2em] flex items-center gap-2"><User size={14} /> Personal Identity</h4></div>
                                            <div className="grid grid-cols-2 gap-y-8 gap-x-6 relative z-10">
                                                <div><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Age</label>{isEditing ? <input type="text" value={editData.age || ''} onChange={(e) => setEditData({...editData, age: e.target.value})} className="w-full px-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none focus:ring-1 ring-[#006a6a]" /> : <p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6]">{selectedRegistration.age || 'N/A'}</p>}</div>
                                                <div>
                                                    <label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Gender</label>
                                                    {isEditing ? <CustomSelect value={editData.gender || ''} onChange={(v) => setEditData({...editData, gender: v})} options={[{label:'Male',value:'Male'},{label:'Female',value:'Female'},{label:'Other',value:'Other'}]} className="bg-white dark:bg-[#43474e] rounded-xl text-sm" /> : <p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6]">{selectedRegistration.gender || 'N/A'}</p>}
                                                </div>
                                                <div className="col-span-2"><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Contact Link</label>{isEditing ? <div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} /><input type="text" value={editData.phone_number || ''} onChange={(e) => setEditData({...editData, phone_number: e.target.value})} className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none focus:ring-1 ring-[#006a6a]" /></div> : <p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6] flex items-center gap-2"><Phone size={14} className="text-[#006a6a]" /> {selectedRegistration.phone_number}</p>}</div>
                                                <div className="col-span-2"><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Secure Email</label>{isEditing ? <input type="email" value={editData.email || ''} onChange={(e) => setEditData({...editData, email: e.target.value})} className="w-full px-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none focus:ring-1 ring-[#006a6a]" /> : <p className="text-sm font-bold text-[#1a1c1e] dark:text-[#e3e2e6] truncate">{selectedRegistration.email || 'N/A'}</p>}</div>
                                                <div className="col-span-2"><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Residence Address</label>{isEditing ? <textarea value={editData.address || ''} onChange={(e) => setEditData({...editData, address: e.target.value})} className="w-full px-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none min-h-[80px] focus:ring-1 ring-[#006a6a]" /> : <p className="text-sm font-medium text-[#43474e] dark:text-[#c4c7c5] leading-relaxed">{selectedRegistration.address || 'N/A'}</p>}</div>
                                            </div>
                                        </div>
                                        {/* Clinical Info */}
                                        <div className="p-8 rounded-[32px] border border-[#e0e2ec] dark:border-[#43474e] bg-white/40 dark:bg-black/10 space-y-8 relative overflow-hidden shadow-inner">
                                            <div className="absolute top-0 right-0 w-24 h-24 bg-[#b3261e]/5 rounded-bl-full -mr-8 -mt-8 opacity-40"></div>
                                            <div className="flex items-center justify-between relative z-10"><h4 className="text-[11px] font-black text-[#b3261e] uppercase tracking-[0.2em] flex items-center gap-2"><Activity size={14} /> Clinical Registry</h4></div>
                                            <div className="grid grid-cols-2 gap-y-8 gap-x-6 relative z-10">
                                                <div className="col-span-2"><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Diagnosis / Complaint</label>{isEditing ? <input type="text" value={editData.chief_complain || ''} onChange={(e) => setEditData({...editData, chief_complain: e.target.value})} className="w-full px-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none focus:ring-1 ring-[#006a6a]" /> : <div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-[#b3261e]"></div><p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6]">{selectedRegistration.chief_complain}</p></div>}</div>
                                                <div>
                                                    <label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Clinical Service</label>
                                                    {isEditing ? <CustomSelect value={editData.consultation_type || ''} onChange={(v) => setEditData({...editData, consultation_type: v})} options={[{label:'Select',value:''}, ...options.types.map(t => ({label:t,value:t}))]} className="bg-white dark:bg-[#43474e] rounded-xl text-sm" /> : <p className="text-sm font-black text-[#1a1c1e] dark:text-[#e3e2e6] uppercase tracking-wider bg-[#e0e2ec] dark:bg-[#43474e] px-3 py-1 rounded-lg w-fit">{selectedRegistration.consultation_type}</p>}
                                                </div>
                                                <div>
                                                    <label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Source / Referrer</label>
                                                    {isEditing ? <CustomSelect value={editData.reffered_by || ''} onChange={(v) => setEditData({...editData, reffered_by: v})} options={[{label:'Select',value:''}, ...options.referred_by.map(r => ({label:r,value:r}))]} className="bg-white dark:bg-[#43474e] rounded-xl text-sm" /> : <p className="text-base font-black text-[#1a1c1e] dark:text-[#e3e2e6]">{selectedRegistration.reffered_by || 'Direct Entry'}</p>}
                                                </div>
                                                <div><label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Accounting Fee</label>{isEditing ? <input type="number" value={editData.consultation_amount || ''} onChange={(e) => setEditData({...editData, consultation_amount: e.target.value})} className="w-full px-4 py-2.5 bg-white dark:bg-[#43474e] border border-[#e0e2ec] dark:border-[#43474e] rounded-xl text-sm font-bold shadow-sm outline-none focus:ring-1 ring-[#006a6a]" /> : <p className="text-xl font-black text-[#1a1c1e] dark:text-[#e3e2e6]">₹ {selectedRegistration.consultation_amount}</p>}</div>
                                                <div>
                                                    <label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Payment Logic</label>
                                                    {isEditing ? <CustomSelect value={editData.payment_method || ''} onChange={(v) => setEditData({...editData, payment_method: v})} options={[{label:'Cash',value:'Cash'},{label:'Online',value:'Online'},{label:'Card',value:'Card'}]} className="bg-white dark:bg-[#43474e] rounded-xl text-sm" /> : <p className="text-sm font-black text-[#1a1c1e] dark:text-[#e3e2e6] uppercase tracking-[0.1em] flex items-center gap-2"><CreditCard size={14} className="text-[#006a6a]" /> {selectedRegistration.payment_method}</p>}
                                                </div>
                                                <div className="col-span-2 pt-2">
                                                    <label className="text-[10px] font-black text-[#43474e]/60 dark:text-[#c4c7c5]/60 uppercase tracking-widest block mb-2">Doctor's Observational Notes</label>
                                                    <div className="p-5 bg-white/60 dark:bg-black/20 border border-[#e0e2ec] dark:border-[#43474e] rounded-[24px] text-sm text-[#43474e] dark:text-[#c4c7c5] font-medium leading-relaxed min-h-[80px] shadow-inner font-sans">{selectedRegistration.doctor_notes || 'No specialized clinical notes recorded for this patient yet.'}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            {selectedRegistration && selectedTrack && (
                <DynamicServiceModal
                    ...
                    onClose={() => { setIsDynamicModalOpen(false); setSelectedTrack(null); }}
                    registration={selectedRegistration}
                    track={selectedTrack}
                    onSuccess={() => {
                        showToast(`Successfully converted to ${selectedTrack.name} patient`, 'success');
                        if (selectedRegistration?.registration_id) {
                            fetchDetails(selectedRegistration.registration_id);
                        }
                        fetchRegistrations();
            ...
            <UpdatePaymentModal 
                ...
                onClose={() => { setIsPaymentModalOpen(false); setSelectedForPaymentFix(null); }}
                registration={selectedForPaymentFix}
                paymentMethods={options.payment_methods || []}
                onSuccess={() => {
                    fetchRegistrations();
                    setToast({ message: 'Payment updated & auto-approved', type: 'success' });
                }}
            />
                ...
                {showLogoutConfirm && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#fdfcff] dark:bg-[#1a1c1e] w-full max-w-sm rounded-[28px] shadow-2xl relative z-10 p-8 text-center border border-[#e0e2ec] dark:border-[#43474e]">
                            <div className="w-16 h-16 rounded-full bg-[#ffdad6] dark:bg-[#93000a] text-[#410002] dark:text-[#ffdad6] mx-auto flex items-center justify-center mb-6"><LogOut size={32} /></div>
                            <h3 className="text-xl font-black text-[#1a1c1e] dark:text-[#e3e2e6] mb-2">Logout</h3>
                            <p className="text-sm font-medium text-[#43474e] dark:text-[#c4c7c5] mb-8">Are you sure you want to log out of your account?</p>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 py-2.5 bg-[#e0e2ec] dark:bg-[#43474e] text-[#1a1c1e] dark:text-[#e3e2e6] rounded-full text-xs font-bold hover:bg-[#c9cdd6] dark:hover:bg-[#5b5e66] transition-all">Cancel</button>
                                <button onClick={() => { logout(); navigate('/login'); }} className="flex-1 py-2.5 bg-[#ba1a1a] text-white rounded-full text-xs font-bold hover:bg-[#93000a] transition-all shadow-md">Logout</button>
                            </div>
                        </motion.div>
                    </div>
                )}
