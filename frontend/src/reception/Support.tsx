  ...
  const [tickets] = useState<SupportTicket[]>([
    {
      ...
      description:
        "There should be an option to write custom notes in billing...",
      status: "In Progress",
    ...
    },
    {
      ...
      description:
        "When I change my profile picture, it does not reflect immediately...",
      status: "Responded",
    ...
    >
      ...
      >
        <div className="space-y-12 z-10">
          <div className="flex items-center gap-4">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-emerald-500 ${isDark ? "bg-[#1C1C1C]" : "bg-emerald-50"}`}
            >
              <HelpCircle size={22} />
            </div>
            <span className="font-medium tracking-[0.2em] text-[12px] uppercase text-slate-500">
              PhysioEZ Core
            </span>
          </div>
        ...
        <div className="space-y-8 w-full flex-1 flex flex-col justify-center py-6">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${isDark ? "bg-[#1C1C1C] text-slate-400" : "bg-slate-50 text-slate-500"}`}
              >
                <Ticket size={14} />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {/* Stat 2: Pending */}
            <div
              className={`p-8 rounded-[32px] border flex flex-col gap-2 ${isDark ? "bg-[#1A1810] border-amber-900/20" : "bg-white border-amber-100 shadow-sm"}`}
            >
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-3 opacity-60">
                  ...
                  <span className="text-sm font-medium uppercase tracking-wider text-amber-600 dark:text-amber-400">
                    Pending
                  </span>
                </div>
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center bg-amber-500/10 text-amber-500`}
                >
                  <Clock size={14} />
                </div>
              </div>
            ...
            {/* Stat 3: Resolved */}
            <div
              className={`p-8 rounded-[32px] border flex flex-col gap-2 ${isDark ? "bg-[#101A12] border-emerald-900/20" : "bg-white border-emerald-100 shadow-sm"}`}
            >
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-3 opacity-60">
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center bg-emerald-500/10 text-emerald-500`}
                >
      ...
      <main className="flex-1 h-screen overflow-y-auto custom-scrollbar relative">
        <div className="p-8 lg:p-12 flex flex-col gap-8">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold tracking-tight">
                      What's New in PhysioEZ
                    </h3>
                    <p className="text-blue-100 text-sm mt-1 opacity-80">
                      Check out the latest updates and improved features in V2.1
                    </p>
                  </div>
                </div>
          ...
          {/* New Ticket Section */}
          <div
            className={`shrink-0 rounded-[40px] border p-8 transition-colors ${isDark ? "bg-[#0A0A0A] border-white/5" : "bg-white border-slate-200/60 shadow-sm"}`}
          >
            <div className="flex items-center gap-3 mb-8">
              <div
                className={`p-2.5 rounded-xl ${isDark ? "bg-emerald-500/10 text-emerald-500" : "bg-emerald-50 text-emerald-600"}`}
              >
                <Plus size={20} />
              </div>
              <h3 className="text-xl font-bold tracking-tight">
            ...
            <div className="flex flex-col lg:flex-row gap-10">
              <div className="flex-1 space-y-4">
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400 px-1">
                  Issue Description
                </label>
                <textarea
                  value={description}
                    ...
                    {files.length > 0 && (
                      ...
                      >
                        {files.map((file, idx) => (
                          <div
                            key={idx}
                            className={`flex items-center justify-between p-3 rounded-xl border ${isDark ? "bg-white/5 border-white/5" : "bg-white border-slate-100 shadow-sm"}`}
                          >
                            <div className="flex items-center gap-3 overflow-hidden">
                              <div className="p-2 rounded-lg bg-blue-500/10 text-blue-500 shrink-0">
                                <Upload size={14} />
                              </div>
                              <span className="text-sm font-medium truncate text-slate-500">
                                {file.name}
                              </span>
                            </div>
                            <button
          ...
          {/* Recent Tickets List */}
          <div
            className={`flex-1 rounded-[40px] border overflow-hidden flex flex-col transition-colors ${isDark ? "bg-[#0A0A0A] border-white/5" : "bg-white border-slate-200/60 shadow-sm"}`}
          >
            <div
              className={`flex items-center justify-between p-8 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`p-2.5 rounded-xl ${isDark ? "bg-blue-500/10 text-blue-500" : "bg-blue-50 text-blue-600"}`}
                >
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`flex items-center gap-3 px-4 py-2.5 rounded-2xl border transition-all ${isDark ? "bg-[#121212] border-white/5 focus-within:border-blue-500/30" : "bg-slate-50 border-slate-100 focus-within:bg-white shadow-inner"}`}
                >
                ...
                <button
                  className={`p-2.5 rounded-xl border ${isDark ? "bg-[#121212] border-white/5" : "bg-white border-slate-200"}`}
                >
                  ...
                  >
                    ...
                    <div className="flex-1 pr-6">
                      <div className="flex items-center gap-3 mb-1">
                        <h4 className="text-sm font-medium text-slate-900 dark:text-slate-100 group-hover:text-blue-500 transition-colors uppercase tracking-tight">
                          {ticket.subject}
                        </h4>
                        <span
                          className={`px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-tighter ${
                    ...
                    <div className="w-[15%] flex justify-center">
                      <span
                        className={`px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 ${
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <button
                  disabled
                  className={`px-4 py-2 rounded-xl border text-xs font-medium uppercase tracking-widest transition-all opacity-30 cursor-not-allowed ${isDark ? "border-white/10" : "border-slate-200 shadow-sm"}`}
                >
                  View Archived
                </button>
                <div className="flex items-center gap-2">
                  <button
                    disabled
                    className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all opacity-30 cursor-not-allowed ${isDark ? "border-white/10" : "border-slate-200 shadow-sm"}`}
                  >
                    <ChevronLeft size={20} />
                  </button>
                  <button
            ...
            >
                ...
                <div>
                  <h2 className="text-3xl font-medium tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-3">
                    ...
                    <span className="text-slate-400 font-mono text-xl font-normal">
                      #{selectedTicket.ticket_no.replace("#", "")}
                    </span>
                  </h2>
              ...
              <div className="p-8 pt-4 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-3">
                  ...
                  <div
                    className={`p-6 rounded-3xl border ${isDark ? "bg-white/[0.01] border-white/5" : "bg-white border-slate-100 shadow-sm"} text-base font-normal leading-relaxed text-slate-600 dark:text-slate-400`}
                  >
                ...
                <div className="space-y-3">
                  ...
                  <div
                    className={`p-6 rounded-3xl ${isDark ? "bg-emerald-500/10" : "bg-emerald-50"} text-base font-medium leading-relaxed text-emerald-600 dark:text-emerald-400`}
                  >
                    {selectedTicket.admin_response ||
                      "Awating response from administrator..."}
                  </div>
                </div>
                ...
                <div className="space-y-3 pb-4">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div
                      className={`w-32 h-40 rounded-2xl border-2 border-dashed flex items-center justify-center p-2 group cursor-pointer ${isDark ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-200"}`}
                    >
