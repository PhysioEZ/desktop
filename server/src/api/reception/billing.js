...
async function fetchCombinedOverview(req, res, branchId) {
    ...
    try {
        ...
        treatmentSql += " ORDER BY last_activity DESC";
        ...
        `;
        let testParams = [branchId, start, end, start, end, start, end];
        if (search) {
            testSql += " AND (t.patient_name LIKE ? OR t.phone_number LIKE ?)";
            testParams.push(`%${search}%`, `%${search}%`);
        }
        testSql += " GROUP BY COALESCE(t.patient_id, CONCAT(t.patient_name, t.phone_number)) ORDER BY last_activity DESC LIMIT 10000";
